...
ProgramEditor::ProgramEditor(kin::Program& program,
  ...
  _visualizerSourceId = visualizerConfig.registerSource(
      "Pose Editor", std::bind(&ProgramEditor::_poseVisualizer, this));
}

std::array<float, 6> ProgramEditor::_poseVisualizer(void) { return _angles; }
...
void ProgramEditor::render() {
    ...
    if (ImGui::BeginMenu("File")) {
      ...
      if (ImGui::BeginMenu("Save")) {
        for (const auto& entry : std::filesystem::directory_iterator(path)) {
          // Give each entry own menu place
          if (ImGui::MenuItem(entry.path().string().c_str()))
            _save(entry.path().string().c_str());
        }
        if (ImGui::MenuItem("Custom")) {
          customFilePopup = true;
          _save("./schemas/custom.json");
        };
        ImGui::EndMenu();
      };

      // Menu for loading different files saved in `./schemas` directory
      if (ImGui::BeginMenu("Load")) {
        for (const auto& entry : std::filesystem::directory_iterator(path)) {
          // Give each entry own menu place
          if (ImGui::MenuItem(entry.path().string().c_str()))
            _load(entry.path().string().c_str());
        }
        ImGui::EndMenu();
      }
      ImGui::EndMenu();
  ...
  if (customFilePopup) ImGui::OpenPopup("Rename");

  if (ImGui::BeginPopupModal("Rename")) {
    ImGui::Text("Give Custom Name:");
    static char name[32] = "";
    static char pathAndName[32 + 7];
    ImGui::InputText("string", name, IM_ARRAYSIZE(name));
    if (ImGui::Button("Save")) {
      ImGui::CloseCurrentPopup();
      customFilePopup = false;
      sprintf(pathAndName, "%s%s", path, name);
      _save(pathAndName);
    };
    ImGui::EndPopup();
  }
  ...
  if (ImGui::Button("New##POSE")) {
    ...
    kin::Puma560 pose;
    for (size_t i = 0; i < 6; ++i) pose.setJointAngle(i, _angles[i]);

    char name[80];
    snprintf(name, sizeof(name), "Pose %lu", _newPoseID++);

    if (!_program.poses.empty()) ++_selectedPoseIdx;
  ...
  for (size_t i = 0; i < _program.poses.size(); ++i) {
    ...
    if (ImGui::Selectable(pose.name.c_str(), isSelected)) {
      _selectedPoseIdx = i;
      for (size_t i = 0; i < 6; ++i) _angles[i] = pose.pose.getJointAngle(i);
  ...
  if (!_program.poses.empty()) {
    ...
    edited |=
        ImGui::InputText("Name##POSE", _curPoseName, sizeof(_curPoseName));

    for (size_t i = 0; i < 6; ++i) degAngles[i] = _angles[i] / PI * 180;
    ...
    for (size_t i = 0; i < 6; ++i) _angles[i] = degAngles[i] / 180 * PI;
  ...
  if (!_program.steps.empty()) {
    if (insertedPose != -1) {
      ...
      for (auto& step : _program.steps) {
        if (step->endPoseIdx >= (size_t)insertedPose) ++step->endPoseIdx;
      }
    }
    if (removedPose != -1) {
      for (size_t i = 0; i < _program.steps.size();) {
        if (_program.steps[i]->endPoseIdx == (size_t)removedPose) {
          ...
          removeStep(i);
          if (_selectedStepIdx > i) --_selectedStepIdx;
          continue;
  ...
  if (ImGui::Button("New##STEP")) {
    ...
    if (!_program.steps.empty()) ++_selectedStepIdx;
    switch (_selectedStepType) {
      ...
      case StepTypes::TrapezoidDriveStep:
        _program.addStep<TrapezoidDrive>(_selectedStepIdx, name,
                                         _selectedPoseIdx);
        break;
  ...
  if (ImGui::BeginCombo("Type", stepTypeNames[_selectedStepType])) {
    for (size_t i = 0; i < numStepTypeNames; ++i) {
      if (ImGui::Selectable(stepTypeNames[i], i == (size_t)_selectedStepType))
        _selectedStepType = i;
      if (i == (size_t)_selectedStepType) ImGui::SetItemDefaultFocus();
    }
    ImGui::EndCombo();
  }
  ...
  if (!_program.steps.empty()) {
    ...
    step->edit();
    if (ImGui::BeginCombo("End pose",
                          _program.poses[step->endPoseIdx].name.c_str())) {
      for (size_t i = 0; i < _program.poses.size(); ++i) {
        if (ImGui::Selectable(_program.poses[i].name.c_str(),
                              i == step->endPoseIdx))
          step->endPoseIdx = i;
        if (i == step->endPoseIdx) ImGui::SetItemDefaultFocus();
      }
      ImGui::EndCombo();
    }
    if (ImGui::Button("Delete##STEP")) removeStep(_selectedStepIdx);
    ImGui::SameLine();
    if (ImGui::ArrowButton("##SwapStepUp", ImGuiDir_Up)) {
      if (_selectedStepIdx > 0) {
        auto stepIter = _program.steps.begin();
        std::iter_swap(stepIter + _selectedStepIdx - 1,
                       stepIter + _selectedStepIdx);
        --_selectedStepIdx;
      }
    }
    ImGui::SameLine();
    if (ImGui::ArrowButton("##SwapStepDown", ImGuiDir_Down)) {
      if (_selectedStepIdx < _program.steps.size() - 1) {
        auto stepIter = _program.steps.begin();
        std::iter_swap(stepIter + _selectedStepIdx,
                       stepIter + _selectedStepIdx + 1);
        ++_selectedStepIdx;
      }
    }
  }
...
void ProgramEditor::_load(const char* path) {
  ...
  for (size_t i = 0; i < j["poses"].size(); ++i) {
    ...
    if (sscanf(_program.poses.back().name.c_str(), "Pose %zu", &parseID) == 1)
      if (parseID >= _newPoseID) _newPoseID = parseID + 1;
  }
  ...
  for (size_t i = 0; i < j["steps"].size(); ++i) {
    ...
    if (sscanf(_program.steps.back()->name.c_str(), "Step %zu", &parseID) == 1)
      if (parseID >= _newStepID) _newStepID = parseID + 1;
  }
}
